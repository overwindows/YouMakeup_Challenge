...
class SparseMaxPool(nn.Module):
    def __init__(self, pooling_counts, N):
        ...
        for c in pooling_counts[1:]:
            poolers.extend(
                [nn.MaxPool1d(3, 2)] + [nn.MaxPool1d(2, 1)
                                        for _ in range(c - 1)]
            )
...
class SparseConv(nn.Module):
    def __init__(self, pooling_counts, N, hidden_size):
        ...
        self.convs.extend([nn.Conv1d(hidden_size, hidden_size, 2, 1)
                          for _ in range(pooling_counts[0])])
        for c in pooling_counts[1:]:
            self.convs.extend(
...
class SparseMeanPool(nn.Module):
    def __init__(self, pooling_counts, N):
        ...
        for c in pooling_counts[1:]:
            poolers.extend(
                [nn.AvgPool1d(3, 2)] + [nn.AvgPool1d(2, 1)
                                        for _ in range(c - 1)]
            )
...
def build_feat2d(cfg):
    # [15,8,8] anet, [15] charades
    pooling_counts = cfg.MODEL.MMN.FEAT2D.POOLING_COUNTS
    ...
    if cfg.MODEL.MMN.FEAT2D.NAME == "conv":
        ...
        # return SparseMeanPool(pooling_counts, num_clips)
    else:
        raise NotImplementedError(
            "No such feature 2d method as %s" % cfg.MODEL.MMN.FEAT2D.NAME)
