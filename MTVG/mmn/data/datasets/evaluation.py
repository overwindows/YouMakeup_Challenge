...
def evaluate(cfg, dataset, predictions, nms_thresh, recall_metrics=(1, 5)):
    ...
    logger.info("Performing {} evaluation (Size: {}).".format(
    ...
    table = [['R@{},IoU@{:.01f}'.format(i, torch.round(j*100)/100)
    ...
    for idx, result2d in tqdm(enumerate(predictions)):   # each video
        score2d = torch.pow(result2d['contrastive'] * 0.5 + 0.5,
                            cfg.TEST.CONTRASTIVE_SCORE_POW) * result2d['iou']
        ...
        for gt_moment, pred_score2d in zip(gt_moments, score2d):  # each sentence
            ...
            candidates, scores = score2d_to_moments_scores(
                pred_score2d, num_clips, duration)
            ...
            for i, r in enumerate(recall_metrics):
                if(r == 1):
                    ...
                    result[str(tot)] = [strftime("%H:%M:%S", gmtime(float(moments[:r][0][0]))), strftime(
                        "%H:%M:%S", gmtime(float(moments[:r][0][1])))]
                    tot = tot + 1
                mious = iou(moments[:r], gt_moment)
                bools = mious[:, None].expand(
                    r, num_iou_metrics) >= iou_metrics
    ...
    for i in range(num_recall_metrics):
        for j in range(num_iou_metrics):
            result_dict['R@{},IoU@{:.01f}'.format(recall_metrics[i], torch.round(
                iou_metrics[j]*100)/100)] = recall_x_iou[i][j]
